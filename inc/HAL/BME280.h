...
typedef enum {
	BME280_ERROR = 0, BME280_SUCCESS = !BME280_ERROR
} BME280_RESULT;
...
private:
	...
	void setOSRST(uint8_t osrs);
	void setOSRSP(uint8_t osrs);
	void setOSRSH(uint8_t osrs);

	BME280_RESULT readCalibration(void);
	BME280_RESULT readUP(int32_t *UP);
	BME280_RESULT readUT(int32_t *UT);
